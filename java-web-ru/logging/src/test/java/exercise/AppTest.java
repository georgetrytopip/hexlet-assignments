...
class AppTest {
    ...
    @Test
    void testUserRegistration() throws IOException, ParseException {
        CloseableHttpClient client = HttpClients.createDefault();
        HttpPost postRequest = new HttpPost(baseUrl + "/users/list");
        List<NameValuePair> params = new ArrayList<>();
        params.add(new BasicNameValuePair("firstName", "George"));
        params.add(new BasicNameValuePair("lastName", "Akhmedov"));
        params.add(new BasicNameValuePair("email", "goshlolka@gmail.com"));
        postRequest.setEntity(new UrlEncodedFormEntity(params));
        CloseableHttpResponse response = client.execute(postRequest);
